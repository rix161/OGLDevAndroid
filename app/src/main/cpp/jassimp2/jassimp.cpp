...
static bool loadEmbeddedTextures(JNIEnv *env, const aiScene* cScene, jobject& jScene)
{
    for (unsigned int i = 0; i < cScene->mNumTextures; i++)
    {
		...
		texparams[2].l = jType;
		if (!createInstance(env, "com/gles/rohit/Common"
									"/jassimp2/AiTexture",
                            "(IILjava/lang/String;)V", texparams, jTex))
		{
			throwException(env, "Cannot find constructor com/gles/rohit/Common"
					"/jassimp2/AiTexture(int, int, String");
		}

        /* copy pixels into memory array */
        jbyteArray jPixels = (jbyteArray) callj(env, jTex, "com/gles/rohit/Common"
				"/jassimp2/AiTexture", "getByteData", "()[B");
		if (jPixels == NULL)
		{
...
static bool loadMeshes(JNIEnv *env, const aiScene* cScene, jobject& jScene)
{
	for (unsigned int meshNr = 0; meshNr < cScene->mNumMeshes; meshNr++)
	{
		...
		if (!callv(env, jMesh, "com/gles/rohit/Common"
				"/jassimp2/AiMesh", "setPrimitiveTypes", "(I)V", setTypesParams))
		{
			return false;
		}
		...
		if (!callv(env, jMesh, "com/gles/rohit/Common"
				"/jassimp2/AiMesh", "allocateBuffers", "(IIZI)V", allocateBuffersParams))
		{
			return false;
		}
		...
		if (cMesh->HasNormals())
		{
			...
			if (!callv(env, jMesh, "com/gles/rohit/Common"
					"/jassimp2/AiMesh", "allocateDataChannel", "(II)V", allocateDataChannelParams))
			{
		...
		for (unsigned int b = 0; b < cMesh->mNumBones; b++)
		{
			...
			for (unsigned int w = 0; w < cBone->mNumWeights; w++)
			{
				...
				if (!createInstance(env, "com/gles/rohit/Common"
						"/jassimp2/AiBoneWeight", jBoneWeight))
				{
					return false;
				}
				...
				if (!callStaticObject(env, "com/gles/rohit/Common"
						"/jassimp2/Jassimp", "wrapMatrix", "([F)Ljava/lang/Object;", wrapParams, jMatrix))
				{
					return false;
				}
...
static bool loadSceneNode(JNIEnv *env, const aiNode *cNode, jobject parent, jobject* loadedNode = NULL)
{
	...
	if (!callStaticObject(env, "com/gles/rohit/Common"
			"/jassimp2/Jassimp", "wrapMatrix", "([F)Ljava/lang/Object;", wrapMatParams, jMatrix))
	{
		return false;
	}
	...
	if (!callStaticObject(env, "com/gles/rohit/Common"
								  "/jassimp2/Jassimp", "wrapSceneNode",
	...
	for (unsigned int m = 0; m < cScene->mNumMaterials; m++)
	{
		...
		for (int ttInd = aiTextureType_DIFFUSE; ttInd < aiTextureType_UNKNOWN; ttInd++)
		{
		 // uvIndex ordering starts from 1, make it 0
			...
			if (!callv(env, jMaterial, "com/gles/rohit/Common"
					"/jassimp2/AiMaterial", "setTextureNumber", "(II)V", setNumberParams))
			{
				return false;
			}
		}
		...
		for (unsigned int p = 0; p < cMaterial->mNumProperties; p++)
		{
				...
				cProperty->mDataLength == 3 * sizeof(float))
			{
				...
				if (!callStaticObject(env, "com/gles/rohit/Common"
						"/jassimp2/Jassimp", "wrapColor3", "(FFF)Ljava/lang/Object;", wrapColorParams, jData))
				{
					return false;
				}
				...
				if (!createInstance(env, "com/gles/rohit/Common"
											"/jassimp2/AiMaterial$Property", "(Ljava/lang/String;IIILjava/lang/Object;)V",
				...
				cProperty->mDataLength == 4 * sizeof(float))
			{
				...
				if (!callStaticObject(env, "com/gles/rohit/Common"
						"/jassimp2/Jassimp", "wrapColor4", "(FFFF)Ljava/lang/Object;", wrapColorParams, jData))
				{
					return false;
				}
			...
			else
			{
				...
				if (!createInstance(env, "com/gles/rohit/Common"
						"/jassimp2/AiMaterial$Property", "(Ljava/lang/String;IIII)V", constructorParams, jProperty))
				{
					return false;
				}
...
static bool loadAnimations(JNIEnv *env, const aiScene* cScene, jobject& jScene)
{
	...
	for (unsigned int a = 0; a < cScene->mNumAnimations; a++)
	{
		...
		if (!createInstance(env, "com/gles/rohit/Common"
				"/jassimp2/AiAnimation", "(Ljava/lang/String;DD)V", newAnimParams, jAnimation))
		{
			return false;
		}
		...
		for (unsigned int c = 0; c < cAnimation->mNumChannels; c++)
		{
			...
			if (!createInstance(env, "com/gles/rohit/Common"
					"/jassimp2/AiNodeAnim", "(Ljava/lang/String;IIIII)V", newNodeAnimParams, jNodeAnim))
			{
				return false;
			}
...
static bool loadLights(JNIEnv *env, const aiScene* cScene, jobject& jScene)
{
	...
	for (unsigned int l = 0; l < cScene->mNumLights; l++)
	{
		...
		if (!callStaticObject(env, "com/gles/rohit/Common"
				"/jassimp2/Jassimp", "wrapColor3", "(FFF)Ljava/lang/Object;", wrapColorParams, jDiffuse))
		{
			return false;
		}
		...
		if (!callStaticObject(env, "com/gles/rohit/Common"
				"/jassimp2/Jassimp", "wrapColor3", "(FFF)Ljava/lang/Object;", wrapColorParams, jSpecular))
		{
			return false;
		}
		...
		if (!callStaticObject(env, "com/gles/rohit/Common"
				"/jassimp2/Jassimp", "wrapColor3", "(FFF)Ljava/lang/Object;", wrapColorParams, jAmbient))
		{
			return false;
		}
		...
		if (!callStaticObject(env, "com/gles/rohit/Common"
				"/jassimp2/Jassimp", "wrapVec3", "(FFF)Ljava/lang/Object;", wrapVec3Params, jPosition))
		{
			return false;
		}
		...
		if (!callStaticObject(env, "com/gles/rohit/Common"
				"/jassimp2/Jassimp", "wrapVec3", "(FFF)Ljava/lang/Object;", wrapVec3Params, jDirection))
		{
			return false;
		}
		...
		if (!createInstance(env, "com/gles/rohit/Common"
									"/jassimp2/AiLight", "(Ljava/lang/String;ILjava/lang/Object;Ljava/lang/Object;FFFLjava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;FF)V",
			params, jLight))
		{
			return false;
		}
...
static bool loadCameras(JNIEnv *env, const aiScene* cScene, jobject& jScene)
{
	...
	for (unsigned int c = 0; c < cScene->mNumCameras; c++)
	{
		...
		DeleteLocalRef refPosition(env, jPosition);
		if (!callStaticObject(env, "com/gles/rohit/Common"
				"/jassimp2/Jassimp", "wrapVec3", "(FFF)Ljava/lang/Object;", wrapPositionParams, jPosition))
		{
			return false;
		}
		...
		if (!callStaticObject(env, "com/gles/rohit/Common"
				"/jassimp2/Jassimp", "wrapVec3", "(FFF)Ljava/lang/Object;", wrapPositionParams, jUp))
		{
			return false;
		}
		...
		if (!callStaticObject(env, "com/gles/rohit/Common"
				"/jassimp2/Jassimp", "wrapVec3", "(FFF)Ljava/lang/Object;", wrapPositionParams, jLookAt))
		{
			return false;
		}
		...
		if (!createInstance(env, "com/gles/rohit/Common"
									"/jassimp2/AiCamera", "(Ljava/lang/String;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;FFFF)V",
			params, jCamera))
		{
			return false;
		}
...
static aiFile* aiFileOpen(C_STRUCT aiFileIO* fio, const char* name, const char* mode) {
    ...
    jbyteArray jByteArray = static_cast<jbyteArray>(callj(opsData.env, opsData.jFileIO, "com/gles/rohit/Common"
																  "/jassimp2/JassimpFileIO", "read",
